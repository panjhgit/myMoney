...
class GameEngine {
    constructor(canvas, ctx, systemInfo, levelId) {
        ...
        // 游戏状态
        this.gameState = {
            level: levelId, score: 0, target: 0, timeLeft: 0, board: [], creatures: [], exits: [], iceBlocks: [], // 冰块系统
            hiddenCreatures: [], // 被冰块隐藏的生物
            layers: [], // 多层网格
            blockRegions: new Map(), // 区域索引
            iceStates: new Map(), // 冰层状态
            selectedCreature: null, isGameOver: false, isPaused: false
        };
        ...
        this.config = {
            ...
            CREATURE_COUNT: 8, HIDDEN_CREATURE_COUNT: 5, ICE_MELT_DELAY: 2000, // 冰块融化延迟
            STEP_DURATION: GAME_CONFIG.STEP_DURATION, ANIMATION_DURATION: GAME_CONFIG.ANIMATION_DURATION
        };

        // 颜色配置
        this.colors = [{
            name: 'red', gradient: 'linear-gradient(135deg, #FF6B6B, #FF8E8E)', shape: 'single', blocks: [[0, 0]]
        }, {
            name: 'blue',
                    ...
                    if (colorData) {
                        ...
                        if (creature) {
                            ...
                            blockData.blocks.forEach(block => {
                                ...
                                if (row >= 0 && row < this.config.BOARD_SIZE && col >= 0 && col < this.config.BOARD_SIZE) {
    ...
    createExit(colorData, position) {
        const exit = {
            color: colorData.name, colorData: colorData, position: position, element: null
        };
    ...
    getCreatureAtPosition(x, y) {
        for (let creature of this.gameState.creatures) {
            ...
            if (x >= element.x && x <= element.x + element.width * this.config.CELL_SIZE && y >= element.y && y <= element.y + element.height * this.config.CELL_SIZE) {
                return creature;
            }
        }
        return null;
    ...
    animateCreatureMove(creature, targetRow, targetCol) {
        ...
        path.forEach((step, index) => {
            ...
            // 添加身体摆动
            walkTimeline.to(creature.element, {
                rotation: "+=3deg", duration: stepDuration * 0.3, ease: "circ.inOut", yoyo: true, repeat: 1
            }, delay);
    ...
    checkForExitMatch(creature) {
        // 检查是否在棋盘边缘
        const isAtEdge = creature.row === 0 || creature.row === this.config.BOARD_SIZE - 1 || creature.col === 0 || creature.col === this.config.BOARD_SIZE - 1;
    ...
    isCreatureAtExit(creatureElement, exitElement) {
        ...
        return creatureCenterX >= exitElement.x && creatureCenterX <= exitElement.x + exitElement.width && creatureCenterY >= exitElement.y && creatureCenterY <= exitElement.y + exitElement.height;
    }
    ...
    drawCreatures() {
        ...
        this.gameState.creatures.forEach(creature => {
            ...
            this.ctx.save();
            this.ctx.translate(x + element.width * this.config.CELL_SIZE / 2, y + element.height * this.config.CELL_SIZE / 2);
            this.ctx.scale(element.scale, element.scale);
            this.ctx.translate(-element.width * this.config.CELL_SIZE / 2, -element.height * this.config.CELL_SIZE / 2);
    ...
    drawExits() {
        ...
        this.gameState.exits.forEach(exit => {
            ...
            this.ctx.fillText(exit.color.charAt(0).toUpperCase(), x + element.width / 2, y + element.height / 2 + 4);
        });
    }
